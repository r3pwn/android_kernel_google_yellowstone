...
const struct file_operations tsu6721drv_chrdev_ops = {
	.owner = THIS_MODULE,
#if 0
	.open = gpsdrv_open,
	.read = gpsdrv_read,
...
#define DEVICE_NAME     "tsu6721"
static int tsu6721drv_major;		/* tsu6721 major number */
static struct class *tsu6721drv_class;	/* tsu6721 class during class_create */
static struct device *tsu6721drv_dev;	/* tsu6721 dev during device_create */
...
//static int tsu6721_major;		/* major number */
...
static void tsu6721_reg_init(struct tsu6721_usbsw *usbsw)
{
	...
	dev_err(&client->dev, " tsu6721_reg_init dev ID: 0x%x\n",
			usbsw->dev_id);
	...
	//if (usbsw->mansw < 0)
		dev_err(&client->dev, "%s, REG_MANUAL_SW1:  %02x\n",
				   ...
				   char *buf)
{
	...
	if (value < 0)
        ...
        dev_err(&client->dev, "%s: DEVICE_TYPE: %02x\n", __func__, value);
				   ...
				   char *buf)
{
	...
	if (device_type1 & DEV_T1_USB_MASK || device_type2 & DEV_T2_USB_MASK) {
                    dev_err(&client->dev, "USB_STATE_CONFIGURED\n");
		return snprintf(buf, 22, "USB_STATE_CONFIGURED\n");
        }

            dev_err(&client->dev, "USB_STATE_NOTCONFIGURED\n");
...
static void tsu6721_set_otg(struct tsu6721_usbsw *usbsw, int state)
{
	...
	case DVT_DEMO:
		if (state == TSU6721_ATTACHED)
			gpio_set_value(TEGRA_GPIO_PK3,1);
		else
			gpio_set_value(TEGRA_GPIO_PK3,0);
		break;
	case DVT1_1:
	case DVT1_2:
	case DVT2:
	...
	default:
		if (state == TSU6721_ATTACHED) {
			...
			dev_info(&client->dev, "%s, set TEGRA_GPIO_PBB7: 1\n",
			    __func__);
		} else {
			gpio_set_value(TEGRA_GPIO_PBB7,0);
			dev_info(&client->dev, "%s, set TEGRA_GPIO_PBB7: 0\n",
			    __func__);
		}
		break;
	}
...
//			(val3 & DEV_T3_CHARGER_MASK)) {
	...
	/* JIG */
	/* APPLE CHARGER */
	}else if ((val3 & DEV_T3_CHARGER_MASK)) {
		pr_info("[MUIC] APPLE Charger Connected\n");
		//pdata->callback(CABLE_TYPE_AC, TSU6721_ATTACHED);
		local_usbsw->last_state.attach = CHARGER_CALL;
...
//			TSU6721_ATTACHED, SW_DHOST);
		...
		mhl_onoff_ex(1);
#endif
#if 0//defined(CONFIG_USB_HOST_NOTIFY)
	/* Audio Dock */
...
static int tsu6721_irq_init(struct tsu6721_usbsw *usbsw)
{
	...
	if (client->irq) {
		ret = request_threaded_irq(client->irq, NULL,
			tsu6721_irq_thread, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
			"tsu6721 micro USB", usbsw);
		if (ret) {
			dev_err(&client->dev, "failed to reqeust IRQ, ret = %d\n", ret);
			return ret;
		}
			 ...
			 const struct i2c_device_id *id)
{
	...
	switch_dev = device_create(tsu6721drv_class, NULL, 0, NULL, DEVICE_NAME);
	if (IS_ERR(switch_dev)) {
		pr_err("[TSU6721] Failed to create device (switch_dev)!, %d\n", IS_ERR(switch_dev));
		pr_err("[Caspar] Failed to create device (switch_dev)!, %d\n", IS_ERR(switch_dev));
	...
	pr_info("[MUIC] %s, hw_id=%d\n",__func__, get_cci_hw_id());
	switch (get_cci_hw_id()){
	case EVT:
		break;
	case DVT_DEMO:
		...
		if (ret < 0)
			dev_err(&client->dev,"charger_enable gpio request failed\n");
		else {
			pr_info("%s set TEGRA_GPIO_PK3 to LOW\n", __func__);
			ret=gpio_direction_output(TEGRA_GPIO_PK3,0);
			if (ret < 0)
				dev_err(&client->dev,"TEGRA_GPIO_PK3 direction output  failed\n");
		}
		break;
	case DVT1_1:
	case DVT1_2:
	case DVT2:
		dev_info(&client->dev, "%s, set TEGRA_GPIO_PBB7 charger_enable\n", __func__ );
		ret = gpio_request(TEGRA_GPIO_PBB7, "charger_enable");
		if (ret < 0)
			dev_err(&client->dev, "charger_enable gpio request failed\n");
		else {
			dev_err(&client->dev, "set TEGRA_GPIO_PBB7 to LOW\n");
			ret=gpio_direction_output(TEGRA_GPIO_PBB7,0);
			if (ret < 0)
			    dev_err(&client->dev, "TEGRA_GPIO_PBB7 direction output  failed\n");
		}
		break;
		default:
		break;
...
static struct of_device_id ti_tsu6721_dts_table[] = {
...
static struct i2c_driver tsu6721_i2c_driver = {
	.driver = {
                    .name = "tsu6721",
                    .owner = THIS_MODULE,
                    .of_match_table = ti_tsu6721_dts_table,
	},
	.probe = tsu6721_probe,
...
static int __init tsu6721_init(void)
{
	 ...
	 */
	tsu6721drv_major = register_chrdev(0, DEVICE_NAME, \
			&tsu6721drv_chrdev_ops);
	if (0 > tsu6721drv_major) {
		pr_err("Error when registering to char dev");
		return TSU6721_ERR_FAILURE;
	}
	pr_err("%s: allocated %d, %d", __func__, tsu6721drv_major, 0);

	/*  udev */
	tsu6721drv_class = class_create(THIS_MODULE, DEVICE_NAME);
	if (IS_ERR(tsu6721drv_class)) {
		pr_err(" Something went wrong in class_create");
		unregister_chrdev(tsu6721drv_major, DEVICE_NAME);
		return TSU6721_ERR_CLASS;
	}
        pr_err(" %s: class_create", __func__);

	tsu6721drv_dev =
		device_create(tsu6721drv_class, NULL, MKDEV(tsu6721drv_major, 0),
				NULL, DEVICE_NAME);
	if (IS_ERR(tsu6721drv_dev)) {
		pr_err(" Error in class_create");
		unregister_chrdev(tsu6721drv_major, DEVICE_NAME);
		class_destroy(tsu6721drv_class);
		return TSU6721_ERR_CLASS;
	}
#endif

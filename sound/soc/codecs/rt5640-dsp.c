...
static const u16 rt5640_dsp_8[][2] = {
	{0x22c8, 0x0021}, {0x22fa, 0x2487}, {0x2301, 0x0000},
};
#define RT5640_DSP_8_NUM (sizeof(rt5640_dsp_8) / sizeof(rt5640_dsp_8[0]))

static const u16 rt5640_dsp_rate_tab[] = {
	0x22c6, 0x22c7, 0x22c8, 0x22fe, 0x22ff, 0x22fa, 0x2301, 0x2306,
};
#define RT5640_DSP_RATE_NUM (sizeof(rt5640_dsp_rate_tab) / \
		sizeof(rt5640_dsp_rate_tab[0]))

static const u16 rt5640_dsp_aec_ns_fens[][2] = {
	{0x22f8, 0x8005}, {0x2303, 0x1901}, {0x2304, 0x4312}, {0x2305, 0x0005},
	...
	{0x2394, 0x4444}, {0x2395, 0x4444}, {0x2396, 0x2000}, {0x2397, 0x3000},
...
static int rt5640_dsp_rate(struct snd_soc_codec *codec, int rate)
{
	...
	case 48000:
		...
		snd_soc_write(codec, RT5640_DSP_PATH1, 0xc000);
		snd_soc_update_bits(codec, RT5640_ASRC_1, 0x4000, 0x4000);
		break;
	case 44100:
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		ret = snd_soc_write(codec, RT5640_PRIV_INDEX,
...
/**
 * rt5640_dsp_array_show - Dump DSP registers in array format.
 * @dev: codec device.
 * @attr: device attribute.
 * @buf: buffer for display.
 *
 * To show non-zero values of all DSP registers in array format.
 *
 * Returns buffer length.
 */

#define RT5640_DSP_REG_ARRAY_DISP_LEN 19
#define RT5640_DSP_REG_ARRAY_RTRN_LEN 20
static ssize_t rt5640_dsp_array_show(struct device *dev,
	...
	for (i = 0; i < tab_num; i++) {
		...
		if ( (i + 1) % 4 != 0) {
			cnt += snprintf(buf + cnt,
				RT5640_DSP_REG_ARRAY_DISP_LEN,
				"{0x%04x, 0x%04x}, ",
				rt5640_dsp_tab[i][0], val);
		} else {
			cnt += snprintf(buf + cnt,
				RT5640_DSP_REG_ARRAY_RTRN_LEN,
				"{0x%04x, 0x%04x},\n",
				rt5640_dsp_tab[i][0], val);
		}
	}
...
static DEVICE_ATTR(dsp_reg_array, 0444, rt5640_dsp_array_show, NULL);
	...
	struct device_attribute *attr, char *buf)
{
	...
	rt5640_dsp_tab = rt5640_dsp_init;
	tab_num = RT5640_DSP_INIT_NUM;
	for (i = 0; i < tab_num; i++) {
		if (cnt + RT5640_DSP_REG_DISP_LEN >= PAGE_SIZE)
			break;
		val = rt5640_dsp_read(codec, rt5640_dsp_tab[i][0]);
	...
	for (i = 0; i < RT5640_DSP_RATE_NUM; i++) {
		if (cnt + RT5640_DSP_REG_DISP_LEN >= PAGE_SIZE)
			break;
		val = rt5640_dsp_read(codec, rt5640_dsp_rate_tab[i]);
		if (!val)
			continue;
		cnt += snprintf(buf + cnt, RT5640_DSP_REG_DISP_LEN,
			"#rnv%04x  #rv%04x  #rd0\n",
			rt5640_dsp_rate_tab[i], val);
	}
...
static DEVICE_ATTR(dsp_reg, 0644, rt5640_dsp_show, dsp_reg_store);
...
int rt5640_dsp_probe(struct snd_soc_codec *codec)
{
	...
	ret = device_create_file(codec->dev, &dev_attr_dsp_reg_array);
	if (ret != 0) {
		dev_err(codec->dev,
			"Failed to create index_reg sysfs files: %d\n", ret);
		return ret;
